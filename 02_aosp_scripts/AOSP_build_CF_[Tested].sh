...
echo "[INFO] Entering source directory..."
cd ~/WORKSPACE/AOSP_15.0.0_36

echo "[INFO] Cleaning wrong manifests if any..."
rm -rf .repo/manifests .repo/manifest.xml || true

echo "[INFO] Initializing repo with Android 15.0.0_r36 manifest..."
repo init -u https://android.googlesource.com/platform/manifest -b android-15.0.0_r36

echo "[INFO] Syncing source (this may take a long time)..."
repo sync -c -j$(nproc --all) --force-sync
...
echo "[INFO] Setting output directory..."
export OUT_DIR=~/WORKSPACE/AOSP_15.0.0_36/out_AOSP15_CF

echo "[INFO] Applying AppArmor fixes (for unprivileged namespaces)..."
sudo sysctl -w kernel.apparmor_restrict_unprivileged_unconfined=0
sudo sysctl -w kernel.apparmor_restrict_unprivileged_userns=0
...
echo "[INFO] Loading build environment..."
source build/envsetup.sh
...
echo "[INFO] Selecting Cuttlefish Auto target..."
...
HOST_OS=linux
HOST_OS_EXTRA=Linux-6.14.0-29-generic-x86_64-Ubuntu-24.04.3-LTS
HOST_CROSS_OS=windows
...
# m
echo "[INFO] Building with all available cores..."
# m -j$(nproc --all)      # The laptop crash
m
...
# echo "[INFO] Installing cvd host package (if not installed)..."
# sudo apt-get install -y cvd-host-package || true

# echo "[INFO] Launching cuttlefish virtual device..."
# launch_cvd
